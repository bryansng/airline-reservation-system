...
public class AuthController {
  ...
  // "hong.sng@ucdconnect.ie") String email,
  ...
  // return new ResponseEntity<>(new SignInResponse("Logged in successfully.", new
  ...
  // autoRegister(@RequestParam(defaultValue = "d d d d d d d") String fullName,
  // @RequestParam(defaultValue = "d@d.d") String email,
  // @RequestParam(defaultValue = "1234") String password,
  // HttpServletRequest request) {
  // RegisterRequest registerRequest = new RegisterRequest(email, password, "d",
  // "d", "123 Road", "123",
  // LocalDateTime.now());
  // ResponseEntity<RegisterResponse> registerResponse =
  // customerService.create(registerRequest);
  ...
  /*
   * /login POST { email: String password: String } returns { customer: Customer
   * object }
   */
